Feature: Limit quota campaign when reach bulk duplicate campaign per hourly printhub
  #sbase_reach_bulkduplicate_limit_quota_campaign
  ...
  Scenario: Check Bulk duplicate when reach quota campaign per hourly #SB_PRB_QPC_90
    Given user login to shopbase dashboard by API
    And user navigate to "Apps" screen
    And select app "Print Hub" on Apps list
    And user navigate to "All Campaigns" screen
