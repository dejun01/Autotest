Feature: Inside - Checkout with Paypal smart button
  #env = sbase_checkout_paypal_smart_button_inside
  Background:
    Given clear all data

  Scenario: Checkout by Paypal smart button - paypal #SB_CHE_PP_14
...
#  Scenario: Checkout by Paypal smart button - debit or credit card #SB_CHE_PP_15
...
#    And close browser
  ...
  Scenario: Checkout by Paypal smart button - paypal with postpurchase item #SB_CHE_PP_16
    Given open shop on storefront
...
#    Cmt lại do chưa handle được case này trên server
#  Scenario: Checkout by Paypal smart button - debit or credit card without post purchase item #SB_CHE_PP_17
#    And open shop on storefront
#    And add products "Post Purchase" to cart then navigate to checkout page
#    When input Customer information
#      | Email            | First Name | Last Name | Address           | Country       | City         | Zip code | State      | Phone      |
